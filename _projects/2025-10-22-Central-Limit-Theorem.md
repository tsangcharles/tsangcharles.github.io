...
class CLTSimulation {
    ...
    drawPopulation() {
        ...
        // Use PDF for preset distributions instead of sampling
        const bins = 50;
        ...
        for (let i = 0; i < bins; i++) {
            const x = min + (i + 0.5) * binWidth; // Center of bin
            histogram[i] = this.getPDF(dist, x);
        }

        // Calculate population statistics from the distribution
        this.populationMean = 0;
        let secondMoment = 0;
        let totalDensity = 0;
        ...
        for (let i = 0; i < bins; i++) {
            ...
            this.populationMean += x * density;
            secondMoment += x * x * density;
            totalDensity += density;
        }
        
        this.populationMean /= totalDensity;
        secondMoment /= totalDensity;
        const variance = secondMoment - this.populationMean * this.populationMean;
        this.populationSD = Math.sqrt(variance);
    ...
    // Get PDF value for preset distributions
    getPDF(dist, x) {
        switch(dist) {
            case 'uniform':
            ...
            case 'exponential':
                const lambda = 0.5; // rate parameter
                return (x >= 0) ? lambda * Math.exp(-lambda * x) : 0;
            
            case 'bimodal':
                const mu1 = 2, sigma1 = 1.5;
                const mu2 = 8, sigma2 = 1.5;
                const pdf1 = (1 / (sigma1 * Math.sqrt(2 * Math.PI))) * 
                    Math.exp(-0.5 * Math.pow((x - mu1) / sigma1, 2));
                const pdf2 = (1 / (sigma2 * Math.sqrt(2 * Math.PI))) * 
                    Math.exp(-0.5 * Math.pow((x - mu2) / sigma2, 2));
                return 0.5 * (pdf1 + pdf2);
            
            case 'skewed':
                // Beta-like distribution (right skewed)
                if (x < 0 || x > 10) return 0;
                const normalized = x / 10; // normalize to [0,1]
                const alpha = 2, beta = 5; // shape parameters
                const betaFunc = 0.0333; // approximation for Beta(2,5)
                return Math.pow(normalized, alpha - 1) * 
                    Math.pow(1 - normalized, beta - 1) / betaFunc / 10;
